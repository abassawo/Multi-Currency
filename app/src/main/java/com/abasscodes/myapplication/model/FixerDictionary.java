...
public class FixerDictionary {
    public HashMap<String, Double> map = new HashMap<>();
    public static HashMap<String, Boolean> permissionMap = new HashMap<>();

    public static void setPermission(String curr, boolean value){
        permissionMap.put(curr, value);
    }

    public FixerDictionary(Rates rates){
    ...
    public static boolean getPermission(String curr) {
        if(permissionMap == null) return false;
        return permissionMap.get(curr);
    }
